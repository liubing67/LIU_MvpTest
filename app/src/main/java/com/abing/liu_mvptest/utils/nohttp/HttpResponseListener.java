 ...
 */
package com.abing.liu_mvptest.utils.nohttp;
...
import com.abing.liu_mvptest.R;
...
public class HttpResponseListener<T> implements OnResponseListener<T> {
    ...
    public void onStart(int what) {
        if (mWaitDialog != null && mActivity!=null && !mWaitDialog.isShowing())
            mWaitDialog.show();
    }
    ...
    public void onFailed(int what, Response<T> response) {
        ...
        if (exception instanceof NetworkError) {// 网络不好
           ToastUtil.show(R.string.error_please_check_network);
        } else if (exception instanceof TimeoutError) {// 请求超时
            ToastUtil.show(R.string.error_timeout);
        } else if (exception instanceof UnKnownHostError) {// 找不到服务器
            ToastUtil.show( R.string.error_not_found_server);
        } else if (exception instanceof URLError) {// URL是错的
            ToastUtil.show(R.string.error_url_error);
